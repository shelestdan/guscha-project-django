...
class UserRepository:
    ...
    async def get_by_phone(phone: str) -> Optional[User]:
        ...
        try:
            ...
            # Пробуем найти пользователя по разным форматам номера
            # 1. Точное совпадение с нормализованным номером
            ...
            # 2. Если не найден и номер начинается с +, пробуем без +
            if normalized_phone.startswith("+"):
                user = await sync_to_async(
                    User.objects.filter(phone=normalized_phone[1:]).first
                )()
                if user:
                    return user

            # 3. Если не найден и номер НЕ начинается с +, пробуем с +
            if not normalized_phone.startswith("+"):
                user = await sync_to_async(
                    User.objects.filter(phone=f"+{normalized_phone}").first
                )()
                if user:
                    return user

            # 4. Пробуем поиск по частичному совпадению (последние 10 цифр)
            ...
            if len(digits_only) >= 10:
                ...
                user = await sync_to_async(
                    lambda: User.objects.filter(
                        Q(phone__endswith=last_10_digits)
                    ).first()
                )()
                if user:
                    return user
    ...
    async def create_telegram_user(
        ...
        try:
            ...
            clean_phone = (
                normalized_phone.replace("+", "").replace(" ", "").replace("-", "")
            )
    ...
    async def update_telegram_info(
        self, user: User, chat_id: str, username: Optional[str] = None
    ) -> None:
    ...
    @async_db_transaction()
    async def verify_telegram(self, user: User) -> None:
