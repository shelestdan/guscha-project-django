...
class ContactHandler(MessageHandler):
    ...
    async def handle(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            await self.message_service.remove_keyboard(
                chat_id=chat_id,
                text="📱 Номер телефона получен, проверяем..."
            )
            
            # Обрабатываем номер телефона
            telegram_phone = contact.phone_number
            is_valid, normalized_phone, error_msg = self.phone_service.process_phone_input(
                phone=telegram_phone,
                context="contact_sharing"
            )
            ...
            if not verification_code:
                ...
                await self._handle_no_verification_code(chat_id, normalized_phone, user_info)
                return
            ...
            # Обрабатываем в зависимости от типа верификации
    ...
    async def _handle_no_verification_code(
        ...
        try:
            ...
            verification_code = await self.verification_service.create_verification_code(
                verification_type='telegram_registration',
                chat_id=chat_id,
                telegram_phone=phone
            )
            
            self.logger.info(f"Создан новый код верификации {verification_code.id} для регистрации через Telegram")
    ...
    async def _handle_login_verification(
        ...
        self.logger.info(f"Успешный вход через Telegram для пользователя {verification_code.user.id}")
    ...
    async def _handle_telegram_registration(
        ...
        self.logger.info(f"Отправлен запрос на подтверждение регистрации для номера {self.phone_service.get_phone_hash(telegram_phone)}")
    ...
    async def _handle_qr_registration(
        ...
        if verification_code.pending_registration and verification_code.pending_registration.phone:
            ...
            if not phones_match:
                await self.message_service.send_error_message(
                    ...
                    error_type='phone_mismatch',
                    telegram_phone=self.phone_service.format_for_display(telegram_phone),
                    registered_phone=self.phone_service.format_for_display(expected_phone)
                )
                return
    ...
    async def _handle_standard_verification(
        ...
        self.logger.info(f"Код {display_code} отправлен пользователю {chat_id} после проверки номера телефона")