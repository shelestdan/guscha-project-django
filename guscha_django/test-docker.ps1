...
$totalTests++
if ((Test-ComposeFile "docker-compose.yml") -and (Test-ComposeFile "docker-compose.dev.yml")) {
    $testsPassed++
}
...
if ($testsPassed -eq $totalTests) {
    ...
    Write-ColorOutput $Yellow "   .\start-docker.ps1 dev    # Режим разработки"
    Write-ColorOutput $Yellow "   .\start-docker.ps1 prod   # Продакшн режим"
} else {
...
Write-ColorOutput $Blue "   Быстрый старт: .\start-docker.ps1"