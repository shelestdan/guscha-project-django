    ...
    def check_security(self, request, identifier: str = None) -> Dict[str, Any]:
        ...
        if SecurityUtils.is_ip_blacklisted(ip_address):
            SecurityUtils.log_security_event(
                "blocked_ip_access", {"ip_address": ip_address}, ip_address=ip_address
            )
            return {"allowed": False, "error": "Доступ запрещен"}
...
class UserViewSet(BaseViewMixin, viewsets.ModelViewSet):
    ...
    def dispatch(self, request, *args, **kwargs):
        print(
            f"DEBUG: UserViewSet.dispatch called - Method: {request.method}, Path: {request.path}"
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            security_check = self.check_security(
                request, f"register_{self.get_client_info(request)['ip_address']}"
            )
            ...
            if hasattr(serializer, "initial_data"):
                ...
                if active_pending:
                    ...
                    if timezone.now() - active_pending.created_at < timezone.timedelta(
                        minutes=5
                    ):
    ...
    def login(self, request):
        ...
        try:
            ...
            login_field = request.data.get(
                "email", ""
            )  # Поле называется email для совместимости
            ...
            is_phone = bool(
                phone_pattern.match(login_field.replace(" ", "").replace("-", ""))
            )
            ...
            if is_phone:
                ...
                try:
                    ...
                    logger.debug(
                        f"Пользователь с телефоном {normalized_phone} не найден"
                    )
            else:
                # Поиск по email
                try:
                    user = User.objects.get(
                        email=login_field.lower().strip(), is_active=True
                    )
                except User.DoesNotExist:
                    logger.debug(
                        f"Пользователь с email {login_field.lower().strip()} не найден"
                    )
    ...
    def logout(self, request):
        """Выход пользователя - полный и безопасный"""
        try:
            ...
            # Сохраняем данные пользователя ДО logout
            user = request.user
            user_email = user.email
            user_id = user.id
            client_info = self.get_client_info(request)
            
            # 1. Удаление всех токенов пользователя (ДО django_logout!)
            Token.objects.filter(user_id=user_id).delete()
            
            # 2. Выход из Django сессии
            django_logout(request)
            
            # 3. Очистка cookie через Response
            response = Response({
                "message": "Успешный выход из системы",
                "success": True
            })
            
            # 4. Очищаем сессионные cookie на стороне клиента
            response.delete_cookie('sessionid')
            response.delete_cookie('csrftoken')
            
            # Логирование для безопасности
            logger.info(f"Пользователь {user_email} вышел из системы с IP: {client_info['ip_address']}")
    ...
    def google_login(self, request):
        ...
        try:
            ...
            allowed_redirect_uris_env = os.environ.get(
                "GOOGLE_OAUTH_ALLOWED_REDIRECT_URIS", ""
            )

            if allowed_redirect_uris_env:
                allowed_redirect_uris = [
                    uri.strip() for uri in allowed_redirect_uris_env.split(",")
                ]
            else:
            ...
            if not redirect_uri or redirect_uri not in allowed_redirect_uris:
                logger.error(
                    f"Invalid redirect_uri: {redirect_uri}. Allowed URIs: {allowed_redirect_uris}"
                )
                return Response(
            ...
            # Проверка безопасности
            security_check = self.check_security(
                request, f"google_login_{self.get_client_info(request)['ip_address']}"
            )
            if not security_check["allowed"]:
                return Response(
                    {"error": security_check["error"]},
            ...
            # Аутентификация пользователя через Django login
            ...
            # Создаем JWT токены для фронтенда (как в обычном login)
            ...
            return Response(
                {
                    ...
                    "token": str(refresh.access_token),  # для обратной совместимости
                    "success": True
                }
            )
    ...
    def me(self, request):
        ...
        try:
            if request.method == "GET":
                ...
                updated_user = self.user_service.update_user(request.user, update_data)
...
def password_reset_request(request):
    ...
    try:
        ...
        result = auth_service.initiate_password_reset(email, client_info=client_info)
...
def telegram_bot_activate(request):
    ...
    try:
        ...
        if not email:
            return Response(
                {"error": "Email обязателен"}, status=status.HTTP_400_BAD_REQUEST
            )
...
def telegram_login_initiate(request):
    """Инициация аутентификации через Telegram (универсальная для входа и регистрации)"""
    try:
        ...
        # Проверка безопасности
        security_check = SecurityUtils.check_rate_limit(f"telegram_auth_{phone_number}")
        if not security_check["allowed"]:
            return Response(
                {"error": "Превышен лимит запросов"},
                status=status.HTTP_429_TOO_MANY_REQUESTS,
        ...
        # Создаем универсальный код верификации
        # Тип 'telegram_auth' - универсальный для входа и регистрации
        # Решение о том, вход это или регистрация, принимается в телеграм боте
        # после получения и проверки номера телефона
        verification_code = TelegramVerificationCode.objects.create(
            telegram_phone=normalized_phone,
            verification_type="telegram_auth",  # Универсальный тип
        ...
        # Генерируем ссылку на телеграм бота
        bot_username = getattr(settings, "TELEGRAM_BOT_USERNAME", "GuschaBot")
        telegram_link = (
            f"https://t.me/{bot_username}?start=auth_{display_code}"
        )

        # Логируем создание кода
        logger.info(
            f"Создан универсальный код верификации {display_code} для номера {normalized_phone}"
        )

        return Response(
            {
                "message": "Запрос на аутентификацию отправлен в Telegram",
                "verification_code": display_code,
                "telegram_link": telegram_link,
                "info": "Перейдите в Telegram бот и поделитесь своим номером телефона для продолжения",
            }
        )
...
def telegram_login_status(request):
    ...
    try:
        ...
        normalized_phone = ValidationUtils.normalize_phone_number(phone_number)
        logger.info(
            f"Проверка статуса входа через Telegram для номера: {normalized_phone}"
        )

        # Ищем код верификации (универсальный или специфичный)
        # Приоритет: telegram_auth > login > telegram_registration
        verification_code = (
        ...
        if not verification_code:
            return Response(
                {"success": False, "error": "Код верификации не найден"},
                status=status.HTTP_404_NOT_FOUND,
            )

        # Проверка, использован ли код
        logger.info(
            f"Код верификации найден: ID={verification_code.id}, тип={verification_code.verification_type}, использован={verification_code.is_used}"
        )

        if verification_code.is_used:
            # Для универсального кода или кода входа - ищем пользователя
            # Пробуем найти пользователя разными способами
            user = User.objects.filter(phone=normalized_phone).first()

            # Если не найден, пробуем без + в начале
            if not user and normalized_phone.startswith("+"):
                user = User.objects.filter(phone=normalized_phone[1:]).first()

            # Если не найден, пробуем с + в начале
            if not user and not normalized_phone.startswith("+"):
                user = User.objects.filter(phone=f"+{normalized_phone}").first()
            if user:
                ...
                operation_type = (
                    "вход"
                    if verification_code.verification_type in ["telegram_auth", "login"]
                    else "регистрацию"
                )
                logger.info(
                    f"Успешная {operation_type} через Telegram для пользователя {user.id}"
                )
                ...
                logger.info(f"Возвращаем ответ с токенами для пользователя {user.id}")
            ...
            # Определяем сообщение в зависимости от типа кода
            if verification_code.verification_type == "telegram_auth":
                message = "Ожидание подтверждения в Telegram"
            elif verification_code.verification_type == "login":
                message = "Ожидание подтверждения входа в Telegram"
            else:
...
def telegram_password_reset_request(request):
    ...
    try:
        ...
        return Response({"message": "Код для сброса пароля отправлен в Telegram"})
...
def telegram_password_reset_confirm(request):
    ...
    try:
        ...
        return Response({"message": "Пароль успешно изменен через Telegram"})
...
def send_current_phone_code(request):
    ...
    try:
        ...
        security_check = SecurityUtils.check_rate_limit(
            f"phone_change_code_{user.id}", max_attempts=3, window_minutes=15
        )
        if not security_check["allowed"]:
            return Response(
        ...
        result = telegram_service.send_phone_change_code(
            user=user, verification_type="phone_change_current"
        )
        ...
        return Response({"message": "Код отправлен в ваш Telegram бот"})
...
def verify_current_phone_code(request):
    ...
    try:
        ...
        return Response({"message": "Текущий номер подтвержден"})
...
def request_phone_change(request):
    ...
    try:
        ...
        if (
            user.phone
            and ValidationUtils.normalize_phone_number(user.phone) == normalized_phone
        ):
            return Response(
        ...
        result = telegram_service.initiate_phone_change(
            user=user, new_phone_number=normalized_phone
        )
...
def create_qr_code(request):
    ...
    try:
        ...
        security_check = SecurityUtils.check_rate_limit(
            f"qr_create_{client_info['ip_address']}", max_attempts=10, window_minutes=60
        )
        if not security_check["allowed"]:
            return Response(
        ...
        result = qr_service.create_qr_code(
            base_url=base_url, pending_registration_id=pending_registration_id
        )
...
def qr_trigger(request, qr_id):
    ...
    try:
        ...
        result = qr_service.process_qr_trigger(
            str(qr_id), client_info.get("ip_address"), client_info.get("user_agent")
        )

        if not result["success"]:
            logger.error(f"QR trigger failed: {result.get('error')}")
            return JsonResponse(
                {"error": result.get("error", "Неверный QR код")}, status=400
            )
        ...
        return JsonResponse({"error": "Ошибка обработки QR кода"}, status=500)
...
def qr_status(request, qr_id):
    ...
    try:
        ...
        if not qr_status:
            return Response(
                {"error": "QR код не найден"}, status=status.HTTP_404_NOT_FOUND
            )
...
def qr_bot_started(request):
    ...
    try:
        ...
        if not SecurityUtils.is_ip_allowed(client_ip):
            return Response(
                {"error": "Неавторизованный доступ"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        qr_service = QRService()
        result = qr_service.mark_bot_started(qr_id, telegram_chat_id)
        ...
        display_code = (
            verification_code.generate_secure_code() if verification_code else None
        )
        return Response(
            {"message": "Запуск бота отмечен", "verification_code": display_code}
        )
...
def link_telegram(request):
    ...
    try:
        ...
        success = telegram_service.link_telegram_to_user(
            request.user, telegram_chat_id, telegram_username
        )
        ...
        return Response({"message": "Telegram аккаунт успешно привязан"})
...
def authenticated_password_reset(request):
    ...
    try:
        ...
        security_check = SecurityUtils.check_rate_limit(
            f"auth_password_reset_{request.user.id}", max_attempts=3, window_minutes=60
        )
        if not security_check["allowed"]:
            return Response(
                {"error": "Превышен лимит запросов на сброс пароля"},
...
def confirm_password_reset_token(request):
    ...
    try:
        ...
        result = telegram_service.confirm_password_reset_with_token(
            token=token, new_password=new_password, client_info=client_info
        )
