...
const fragmentShader = `
  ...
  void main() {
    ...
    // Cover-like scaling для сохранения пропорций изображения
    vec2 coverUv = uv;
    if (uContainerAspect > uImageAspect) {
      // Контейнер шире изображения - обрезаем по высоте
      float scale = uContainerAspect / uImageAspect;
      coverUv.y = (uv.y - 0.5) / scale + 0.5;
    } else {
      // Контейнер уже изображения - обрезаем по ширине
...
const GridDistortion = ({
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      // Обновляем aspect ratio контейнера для шейдера
