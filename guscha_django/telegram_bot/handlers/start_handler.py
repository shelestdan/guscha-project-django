...
class StartHandler(CommandHandler):
            ...
            if args:
                ...
                if param:
                    if param.startswith("auth_"):
                        # Универсальный код аутентификации (может быть и вход, и регистрация)
                        verification_id = param[5:]  # Убираем префикс 'auth_'
                        self.logger.info(f"Обнаружен auth параметр: {verification_id}")
                        ...
                        verification_code = param[
                            13:
                        ]  # Убираем префикс 'phone_change_'
    ...
    async def _handle_phone_change_start(
        self, update: Update, verification_code: str
    ) -> None:
        ...
        self.logger.info(
            f"Обработка смены номера для chat_id={chat_id}, code={verification_code}"
        )
        ...
        try:
            ...
            if (
                not phone_change_code
                or phone_change_code.verification_type != "phone_change_new"
            ):
    ...
    async def _handle_qr_code_start(self, verification_code, chat_id: str) -> None:
        """Обрабатывает запуск через QR-код."""
        try:
            # Пытаемся связать QR-код с существующим пользователем
            if not verification_code.user:
                self.logger.info(
                    f"QR-код {verification_code.id} не связан с пользователем, ищем пользователя с chat_id={chat_id}"
                )

                existing_user = await self.user_service.get_user_by_chat_id(chat_id)
                if existing_user:
                    await self.verification_service.update_verification_code(
                        verification_code=verification_code, user=existing_user
                    )
                    self.logger.info(
                        f"QR-код {verification_code.id} успешно связан с пользователем {existing_user.id}"
                    )
            ...
            await self.verification_service.handle_qr_code_verification(
